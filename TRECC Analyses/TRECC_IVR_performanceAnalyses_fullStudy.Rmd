...
message(paste("Currently across all tokens:",nrow(tokenDistribution_A),"unique token As,",nrow(tokenDistribution_B), "unique token Bs, and",nrow(tokenDistribution_C),"unique token Cs have been used in at least one question with at least one student. There are",nrow(tokenDistribution_distractorBC),"unique combinations of Tokens B and C."))
```
...
###Who passed Unit 1?
####The table below displays the four trials for unit 1 and the average questions students attempted
```{r, echo = F}

#this makes the raw data one line for each attempt
UASdata_learningCurve_trial = UASdata %>%
  select(treccId_phoneId, UAS.unit_id, cmsQuestions.trial_id, cmsQuestions.question_number, UAS.created_at, UAS.id, UAS.correct) %>%
...
#need to cumsum a column of "new trial"
  mutate(opportunity = row_number()) %>%
  arrange(treccId_phoneId, cmsQuestions.trial_id, opportunity) %>%
  mutate(rollingAverage = cumsum(UAS.correct) / seq_along(opportunity),
         promotionReady = ifelse(rollingAverage >= .6 & opportunity >= 15, 1, 0)) %>%
  ungroup() %>%
  group_by(treccId_phoneId) %>%
  mutate(maxUnit = max(UAS.unit_id),
         passedUnit1 = as.character(ifelse(maxUnit > 1, 1, 0))) %>%
  ungroup() %>%
  group_by(UAS.unit_id, cmsQuestions.trial_id, opportunity) %>%
  mutate(userCountPerOpportunity = n_distinct(treccId_phoneId)) %>%
  ungroup()

UASdata_unit1Promotion = UASdata_learningCurve_trial %>%
  filter(passedUnit1 == 1,
         UAS.unit_id == 1) %>%
  arrange(treccId_phoneId, cmsQuestions.trial_id, opportunity) %>%
  mutate(promotionPoint = ifelse(treccId_phoneId == lag(treccId_phoneId) & cmsQuestions.trial_id == lag(cmsQuestions.trial_id) & promotionReady == 1 & lag(promotionReady == 0),1,0)) %>%
  filter(promotionPoint == 1) %>%
  ...
  summarize(avgQuestionsBeforePromotion = round(mean(opportunity),2),
            sdQuestionsBeforePromotion = round(sd(opportunity),2),
            avgMastery = round(mean(rollingAverage),2),
            sdMastery = round(sd(rollingAverage),2),
            nStudents = n_distinct(treccId_phoneId))

datatable(
  UASdata_unit1Promotion,
  rownames = FALSE,
  filter = "top",
  extensions = 'FixedColumns',
  options = list(
...
UASdata_learningCurve_trial_oneLinePerOpp = UASdata_learningCurve_trial %>%
  #select(treccId_phoneId, UAS.unit_id, cmsQuestions.trial_id, UAS.created_at, UAS.id, UAS.correct) %>%
  #arrange(treccId_phoneId, cmsQuestions.trial_id, UAS.created_at) %>%
  group_by(passedUnit1, treccId_phoneId, cmsQuestions.trial_id, opportunity) %>%
  mutate(errorRate = mean(errorRate)) %>%
   ungroup()
  # group_by(treccId_phoneId) %>%
...
###Learning Curves
####Data filtered for unit 1 content with 10+ child users per opportunity.
###{.tabset}
####plot
```{r, echo = F}
# #ggplotly(
...
UASdata_learningCurve_trial_oneLinePerOpp %>%
  ...
  ylim(0, 1) +
  labs(x = "opportunity count", y = "avg error rate", col = "passed\nunit 1") +
  stat_smooth(
    method = "auto" 
    #text = paste("n_users: ", unique(UASdata_learningCurve_trial_oneLinePerOpp$userCountPerOpportunity))
) +
